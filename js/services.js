var MainService = (function(){
	...
	var journalDatas = [
		...
		},
		{
			itemCodes: {
				item33: {
					...
					pointPosition: "top: 44%; left: 12%",
					itemImage: {
						src: "img/wreck-diver/items/journal/.jpg",
						text: "Mast collar",
						textPosition: "top: 14%; left: 23%;"
					},
		...
		},
		{
			itemCodes: {
				item141: {
					itemName: "Cathead",
					itemDetail: "The cathead is a wooden beam extending out from the bow of the ship, used to support the anchor. Have you located the anchor yet? (hint: look down!)",
	...
	function activeJournal() {
		...
		dive1ItemsDiscoveredElement.css('z-index', 1);
	  	...
	  	dive1ItemsDiscoveredElement.on('click', function(event){
			...
			bindDatasToJournalByItem('item' + $(this).attr('data-item'));
	...
	function bindDatasToJournalByItem(itemCode) {
		journalDatas.forEach(function(journalData){
			var journal = journalData.itemCodes[itemCode];
